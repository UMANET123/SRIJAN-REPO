    ...
    try {
      ...
      if (appname) {
        ...
        totalRecords = await client.query(
          `SELECT count(*)  FROM public.subscriber_consent consent inner join apps_metadata app on consent.app_id=app.app_id and consent.developer_id=app.developer_id where uuid=($1) 
        ...
        record = await client.query(
          `SELECT consent.app_id, appname, consent.developer_id, scopes FROM public.subscriber_consent consent inner join apps_metadata app on consent.app_id=app.app_id and consent.developer_id=app.developer_id where uuid=($1) 
            and appname like ($2) and status=($3) and scopes IS NOT NULL order by consent.created ${order} LIMIT ($4) OFFSET ($5)`,
        ...
        record = await client.query(
          `SELECT consent.app_id, appname, consent.developer_id, scopes FROM public.subscriber_consent as consent inner join apps_metadata as app on consent.app_id=app.app_id and consent.developer_id=app.developer_id where uuid=($1) 
            and status=($2) and scopes IS NOT NULL order by consent.created ${order} LIMIT ($3) OFFSET ($4)`,
      ...
      if (record.rows[0]) {
        return callback(200, {
          page,
          limit,
          resultcount: totalRecords.rows[0].count,
          apps: record.rows
        });
      } else {
        return callback(200, {
...
async function validateRedirectTxn({
  subscriber_id,
  transaction_id,
  app_id,
  callback
